...
$access = false;
//access for client
$clients_id = WPC()->assigns()->get_assign_data_by_object( 'feedback_wizard', $wizard_id, 'client' );
if ( is_array( $clients_id ) && in_array( $user_id, $clients_id) ) {
    ...
    $groups_id = WPC()->assigns()->get_assign_data_by_object( 'feedback_wizard', $wizard_id, 'circle' );
    if (is_array( $groups_id ) && 0 < count( $groups_id ) ) {
        foreach( $groups_id as $group_id ) {
            $clients_id = WPC()->groups()->get_group_clients_id( $group_id );
            if ( is_array( $clients_id ) && in_array( $user_id, $clients_id) ) {
                $access = true;
                break;
            }
...
<div class="wrap">
    <div class="feedback_wizard">
        <?php if ( isset( $wizard_data['items'] ) && 0 < count( $wizard_data['items'] ) ) { ?>
            ...
            <h1 class="wizard_title"><?php _e( 'Feedback for:', WPC_CLIENT_TEXT_DOMAIN ) ?> <?php echo stripslashes( $wizard_data['name'] ) ?></h1>
                ...
                <div id="wizard" style="min-height: 400px;">
                    <?php foreach(  $wizard_data['items'] as $item ) {
                        ...
                        <fieldset class="sectionwrap">
                            <h3 class="item_name"><?php echo stripslashes( $item['name'] ) ?></h3>
                            <div class="postbox">
                                <div class="inside">
                                    ...
                                    <div class="wizard_comment">
                                        <label style="width: 100%; float:left;">
                                            <?php _e( 'Your Comment:', WPC_CLIENT_TEXT_DOMAIN ) ?>
                            ...
                            <div class="action_buttons" id="action_buttons_<?php echo $item['item_id'] ?>" >
                                <?php $wizard_feedback_type = ( isset( $wizard_data['feedback_type'] ) && '' != $wizard_data['feedback_type']  && '-1' != $wizard_data['feedback_type'] ) ? $wizard_data['feedback_type'] : '-1';
                                ...
                                if ( '-1' != $wizard_feedback_type ) {
                                    ...
                                    switch( $wizard_feedback_type['type'] ) {
                                        case 'radio':
                                            if ( is_array( $wizard_feedback_type['options'] ) && 0 < count( $wizard_feedback_type['options'] ) ) {
                                                foreach( $wizard_feedback_type['options'] as $num=>$name ) {
                                                    ...
                                                    echo '<label><input type="radio" class="wpc_fw_radio" name="feedback[items][' . $item['item_id'] . '][item_feedback]" value="' . $name . '" ' . $checked . ' /> ' . stripslashes( $name ) . '</label>';
                                                }
                                        ...
                                        case 'checkbox':
                                            if ( is_array( $wizard_feedback_type['options'] ) && 0 < count( $wizard_feedback_type['options'] ) ) {
                                                foreach( $wizard_feedback_type['options'] as $num=>$name ) {
                                                    ...
                                                    echo '<label><input type="checkbox" class="wpc_fw_checkbox" name="feedback[items][' . $item['item_id'] . '][item_feedback][]" value="' . $name . '" ' . $checked . ' /> ' . stripslashes( $name ). '</label>' . $align;
                                                }
                                        ...
                                        case 'selectbox':
                                            if ( is_array( $wizard_feedback_type['options'] ) && 0 < count( $wizard_feedback_type['options'] ) ) {
                                                ...
                                                foreach( $wizard_feedback_type['options'] as $num=>$name ) {
                                                    ...
                                                    echo '<option value="' . $name . '" ' . $selected . ' /> ' . stripslashes( $name ) . ' </option>';
                                                }
                                                echo '</select>';
                                        ...
                                        case 'button':
                                            if ( is_array( $wizard_feedback_type['options'] ) && 0 < count( $wizard_feedback_type['options'] ) ) {
                                                foreach( $wizard_feedback_type['options'] as $num=>$name ) {
                                                    echo '<label><input type="button" class="wpc_fw_button" name="feedback[items][' . $item['item_id'] . '][item_feedback]" onclick="jQuery(this).FeedbackClick( \'' . $item['item_id'] . '\', \'button\', \' ' . $name . ' \' );" value="' . stripslashes( $name ) . '" /></label>';
                                                }
                                    ...
                                    <input type="button" class="wpc_button wpc_fw_approve" name="approve" onclick="jQuery(this).FeedbackClick( '<?php echo $item['item_id'] ?>', 'button', 'Approve' );" value="<?php _e( 'Approve', WPC_CLIENT_TEXT_DOMAIN ) ?>" />
                                    <input type="button" class="wpc_button wpc_fw_not_sure" name="not_sure" onclick="jQuery(this).FeedbackClick( '<?php echo $item['item_id'] ?>', 'button', 'Not sure' );" value="<?php _e( 'Not sure', WPC_CLIENT_TEXT_DOMAIN ) ?>" />
                                    <input type="button" class="wpc_button wpc_fw_not_approve" name="not_approved" onclick="jQuery(this).FeedbackClick( '<?php echo $item['item_id'] ?>', 'button', 'Not approved' );" value="<?php _e( 'Not approved', WPC_CLIENT_TEXT_DOMAIN ) ?>" />
                                <?php } ?>
                            </div>
                        </fieldset>
                    <?php } ?>
                    ...
                    <fieldset class="sectionwrap">
                        <h3 class="item_name"><label for="wpc_feedback_final_comment"><?php _e( 'Final comment', WPC_CLIENT_TEXT_DOMAIN ) ?></label></h3>
                        <div class="postbox">
                            <div class="inside">
                    ...
                    <div class="wizard_buttons">
                        <input type="submit" name="send" id="send" class="wpc_submit" value="<?php _e( 'Send Your Feedback', WPC_CLIENT_TEXT_DOMAIN ) ?>" style="display: none;" />
                        <input type="button" name="close" id="close" class="wpc_button" value="<?php _e( 'Close', WPC_CLIENT_TEXT_DOMAIN ) ?>" />
                    </div>
                </div>
            </form>
        <?php } ?>
