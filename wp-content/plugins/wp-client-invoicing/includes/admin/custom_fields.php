...
global $wpdb;
$fields = array();
$wpc_custom_fields = WPC()->get_settings( 'invoice_cf' );
$types = array();
$i = 0;
...
if ( isset($_REQUEST['_wp_http_referer']) ) {
    ...
    $redirect = get_admin_url(). 'admin.php?page=wpclients_invoicing&tab=custom_fields';
}
...
if ( isset( $_GET['action'] ) ) {
    switch ( $_GET['action'] ) {
        ...
        case 'delete':
            $ids = array();
            if ( isset( $_GET['name'] ) ) {
                check_admin_referer( 'wpc_inv_cf_delete' .  $_GET['name'] . get_current_user_id() );
                $ids = (array) $_GET['name'];
            ...
            if ( count( $ids ) ) {
                foreach ( $ids as $item_id ) {
                    $client_ids = get_posts(
                        array( 'role' => 'wpc_client', 'meta_key' => $item_id, 'fields' => 'ID', )
                    );
                ...
                WPC()->settings()->update( $wpc_custom_fields, 'invoice_cf' );
...
class WPC_Invoice_CF_List_Table extends WP_List_Table {
    ...
    function column_options( $item ) {
        ...
        $html .= ' />&nbsp;&nbsp;' . __( 'Readonly', WPC_CLIENT_TEXT_DOMAIN ) . '<br />' ;
        $html .= '<input type="checkbox" disabled ';
        $html .= ( isset( $item['required'] ) && '1' == $item['required'] ) ? 'checked' : '' ;
        $html .= ' />&nbsp;&nbsp;' . __( 'Required', WPC_CLIENT_TEXT_DOMAIN ) ;
    ...
    function column_title( $item ) {
        $actions = array();
        $actions['edit'] = '<a href="admin.php?page=wpclients_invoicing&tab=inv_custom_field_edit&edit=' . $item['name'] . '">' . __( 'Edit', WPC_CLIENT_TEXT_DOMAIN ) . '</a>' ;
        $actions['delete'] = '<a onclick=\'return confirm("' . __( 'Are you sure to delete this Custom Filed?', WPC_CLIENT_TEXT_DOMAIN ) . '");\' href="admin.php?page=wpclients_invoicing&tab=custom_fields&_wpnonce=' . wp_create_nonce( 'wpc_inv_cf_delete' . $item['name'] . get_current_user_id() ) . '&action=delete&name=' . $item['name'] . '&_wp_http_referer=' . urlencode( wp_unslash( $_SERVER['REQUEST_URI'] ) ) . '" >' . __( 'Delete Permanently', WPC_CLIENT_TEXT_DOMAIN ) . '</a>' ;

        $title = ( isset( $item['title'] ) ) ? htmlspecialchars( $item['title'] ) : __( 'No Name', WPC_CLIENT_TEXT_DOMAIN ) ;

        return sprintf('%1$s %2$s', '<span class="this_name" id="field_' . $item['name'] . '">' . $title . '</span>', $this->row_actions( $actions ) );
    }
...
$ListTable = new WPC_Invoice_CF_List_Table( array(
...
<div class="wrap">
    ...
    <div id="wpc_container">
        ...
        <div class="wpc_tab_container_block wpc_inv_custom_fields" style="position: relative;">
            <a href="admin.php?page=wpclients_invoicing&tab=inv_custom_field_edit&add=1" class="add-new-h2">
                    ...
                    */
                    ...
                    jQuery( '#sortable' ).bind( 'sortupdate', function(event, ui) {
                        ...
                        jQuery.ajax({
                            ...
                            data: 'action=change_inv_custom_field_order&type=inv&new_order=' + new_order,
