...
$for_client = ( isset( $_GET['for_client'] ) ) ? '&for_client=' . $_GET['for_client'] : '';

//save settings
if ( isset( $_POST['wpc_scheme_settings'] ) && isset( $_POST['wpc_style_settings'] ) && is_array( $_POST['wpc_style_settings'] ) ) {

    $wpc_customize->save_style_settings( $_POST['wpc_scheme_settings'], $_POST['wpc_style_settings'] );

    WPC()->redirect( admin_url() . 'admin.php?page=wpclients_customize&msg=u' . $for_client );
}


//reset settings
if ( isset( $_GET['reset'] ) && 'true' == $_GET['reset']  ) {
    $wpc_customize->reset_style_settings();

    WPC()->redirect( admin_url() . 'admin.php?page=wpclients_customize&msg=r' . $for_client );
}

$excluded_clients = "'" . implode( "','", WPC()->members()->get_excluded_clients() ) . "'";
$all_clients = $wpdb->get_results( "SELECT u.ID as id, u.user_login as username
    FROM {$wpdb->users} u
...
if( count( $all_clients ) ) {
    $client_id = ( isset( $_GET['for_client'] ) && (int)$_GET['for_client'] > 0 ) ? (int)$_GET['for_client'] : $all_clients[0]['id'];
    $iframe_url = add_query_arg(
        array(
            'hub_preview' => $client_id,
            'nonce' => wp_create_nonce('customizer' . $client_id )
        ), set_url_scheme( WPC()->get_hub_link() ) );
} else {
    $iframe_url = get_home_url();
}

$style_schemes = $wpc_customize->get_style_schemes();
...
<!--                <a class="back button" onclick='return confirm("<?php _e( 'Are you sure to reset Custom Styles?', WPC_CLIENT_TEXT_DOMAIN ) ?>");' href="<?php echo admin_url( 'admin.php?page=wpclients_customize&reset=true' ) ?>"><?php _e( 'Reset', WPC_CLIENT_TEXT_DOMAIN ) ?></a>-->
            ...
            <div tabindex="-1" class="wp-full-overlay-sidebar-content accordion-container">
                ...
                <div id="properties_editor_box" style="display: none;" >
                    <div id="properties_editor" >
                        <?php echo $wpc_customize->_editor_html() ?>
                    </div>
                </div>

                <div id="block_for_select_client">
                    <span><?php printf( __( 'For %s: ', WPC_CLIENT_TEXT_DOMAIN ), WPC()->custom_titles['client']['s'] ) ?></span>
                    <select name="for_client" id="for_client">
                        <?php
                        if ( isset( $_GET['for_client'] ) ) {
                            $selected_client = $_GET['for_client'] ;
                        } elseif( isset( $client[0] ) ) {
                        ...
                        if ( is_array( $all_clients ) && 0 < count( $all_clients ) ) {
                            foreach( $all_clients as $client ) {
                                $selected = ( $client['id'] == $selected_client ) ? ' selected="selected"' : '';
                                echo '<option value="' . $client['id'] . '" ' . $selected . ' >' .  $client['username'] . '</option>';
                            }
                        }
                        ?>
                    </select>
                ...
                <div class="accordion-section" id="customize-info">
                    ...
                    <div id="theme_header">
                        ...
                        if ( isset( $first_scheme_key[0] ) ) {
                             echo $wpc_customize->get_sections_header( $first_scheme_key[0] );
                        }
                        ?>
                    </div>
                ...
                <div id="wpc-customize-theme-controls">
                    <?php
                        echo $wpc_customize->get_sections( '_default_scheme' );
                    ?>
                </div>
        ...
        <div id="element_html" style="display: none;"><?php echo $wpc_customize->_element_html() ?></div>
                    ...
                    jQuery( document ).ready( function() {
                        ...
                        var allProperties2 = new Array(
                         ...
                         var allProperties = {
                             <?php
                                $i = 1;
                                $count = count( $wpc_customize->all_properties );
                                foreach( $wpc_customize->all_properties as $key => $type ) {
