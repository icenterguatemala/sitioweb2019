...
if ( isset($_REQUEST['_wp_http_referer']) ) {
    ...
    $redirect = get_admin_url(). 'admin.php?page=wpclients_content&tab=files_tags';
}
...
if( !empty( $_POST['wpc_action'] ) ) {
    switch( $_POST['wpc_action'] ) {
        ...
        case 'create_file_tag':
            ...
            if ( !$term_info = term_exists($term, 'wpc_file_tags') )
                $term_info = wp_insert_term($term, 'wpc_file_tags');
            else
...
if ( isset( $_GET['action'] ) ) {
    switch ( $_GET['action'] ) {
        case 'delete':
            ...
            if ( count( $ids ) && ( current_user_can( 'wpc_admin' ) || current_user_can( 'administrator' ) || current_user_can( 'wpc_delete_file_tags' ) ) ) {
                foreach ( $ids as $tag_id ) {
                    wp_delete_term( $tag_id, 'wpc_file_tags' );
                }
...
class WPC_File_Tags_Table extends WP_List_Table {
    ...
    function column_name( $item ) {
        ...
        if( $item['count'] > 0 ) {
            $actions['view'] = '<a class="various" target="_blank" href="admin.php?page=wpclients_content&tab=files&change_filter=tag&filter_tag=' . $item['id'] . '" title="" >' . __( 'View', WPC_CLIENT_TEXT_DOMAIN ). '</a>';
        }

        if ( current_user_can( 'wpc_admin' ) || current_user_can( 'administrator' ) || current_user_can( 'wpc_delete_file_tags' ) )  {
            $actions['delete'] = '<a onclick=\'return confirm("' .  __( 'Are you sure you want to delete this file tag?', WPC_CLIENT_TEXT_DOMAIN ) . '");\' href="admin.php?page=wpclients_content&tab=files_tags&action=delete&tag_id=' . $item['id'] . '&_wpnonce=' . wp_create_nonce( 'wpc_file_tag_delete' . $item['id'] . get_current_user_id() ) . '&_wp_http_referer=' . urlencode( stripslashes_deep( $_SERVER['REQUEST_URI'] ) ) . '" >' . __( 'Delete Permanently', WPC_CLIENT_TEXT_DOMAIN ) . '</a>';
        }
    ...
    function extra_tablenav( $which ){
        if ( 'top' == $which ) {
            $this->search_box( __( 'Search File Tags', WPC_CLIENT_TEXT_DOMAIN ), 'search-submit' );
        }
    }
...
$ListTable = new WPC_File_Tags_Table( array(
...
$per_page   = WPC()->admin()->get_list_table_per_page( 'wpc_file_tags_per_page' );
...
<div class='wrap'>
    ...
    <div id="wpc_container">
        ...
        if ( isset( $_GET['msg'] ) ) {
            switch( $_GET['msg'] ) {
                case 'd':
                    echo '<div id="message" class="updated wpc_notice fade"><p>' . __( 'File Tag(s) are Deleted.', WPC_CLIENT_TEXT_DOMAIN ) . '</p></div>';
                    break;
                case 'rat':
                    echo '<div id="message" class="updated wpc_notice fade"><p>' . __( 'File Tag reassigned successfully.', WPC_CLIENT_TEXT_DOMAIN ) . '</p></div>';
                    break;
                case 'n_rat':
                    echo '<div id="message" class="error wpc_notice fade"><p>' . __( 'File Tag was not reassigned.', WPC_CLIENT_TEXT_DOMAIN ) . '</p></div>';
                    break;
                case 'wt':
