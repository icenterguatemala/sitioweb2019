...
class Mandrill {
    ...
    public function __construct($apikey=null) {
        ...
        add_action( 'http_api_curl', array( &$this, 'follow_location_opt' ), 1000, 3  );
    ...
    public function follow_location_opt( $handle, $r, $url ) {
    ...
    public function call($url, $params) {
        ...
        $ch = $this->ch;
        $response = wp_remote_post( $this->root . $url . '.json', array(
            'timeout'       => 45,
            'redirection'   => 5,
            'httpversion'   => '1.0',
            'blocking'      => true,
            'sslverify'     => false,
            'user-agent'    => 'Mandrill-PHP/1.0.54',
        ...
        if ( is_wp_error( $response ) ) {
            ...
            $response_body = !empty( $response['body'] ) ? $response['body'] : '';
            $result = json_decode($response_body, true);
            if( $result === null )
                return 'We were unable to decode the JSON response from the Mandrill API: ' . $response_body;
            return $result;
        }
    }
