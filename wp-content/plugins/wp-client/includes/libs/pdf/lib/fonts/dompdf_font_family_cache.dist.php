...
if ( defined('WPC_CLIENT_EXTERNAL_FONTS_DIR') ) {
	$wpc_external_font_dir = WPC()->get_upload_dir( WPC_CLIENT_EXTERNAL_FONTS_DIR );

	if( is_dir( $wpc_external_font_dir ) && file_exists( $wpc_external_font_dir . 'fonts.php' ) ) {
		$new_array = include( $wpc_external_font_dir . 'fonts.php' );
		if( is_array( $new_array ) ) {
			$default_array = array_merge( $default_array, $new_array );
		}
	}
}
