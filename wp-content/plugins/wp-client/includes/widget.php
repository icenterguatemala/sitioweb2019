...
class wpc_client_widget extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array( 'classname' => 'wpc_widget_login', 'description' => sprintf( __( 'Allow %s to login/Logout.', WPC_CLIENT_TEXT_DOMAIN ), WPC()->custom_titles['client']['s'] ) );
        parent::__construct( 'wpc_client_widget', WPC()->plugin['title'] . __( ': Login/Logout', WPC_CLIENT_TEXT_DOMAIN ), $widget_ops );
    }
    ...
    function widget( $args, $instance ) {
        ...
        $instance['text_login_place'] = ( isset( $instance['text_login_place'] ) && !empty( $instance['text_login_place'] ) ) ? $instance['text_login_place'] : '';
        $instance['text_pass_place'] = ( isset( $instance['text_pass_place'] ) && !empty( $instance['text_pass_place'] ) ) ? $instance['text_pass_place'] : '';

        $title                  = apply_filters( 'widget_title', $instance['title'] );
        $text_login             = apply_filters( 'widget_title', $instance['text_login'] );
        $text_login_place       = apply_filters( 'widget_title', $instance['text_login_place'] );
        $text_pass              = apply_filters( 'widget_title', $instance['text_pass'] );
        $text_pass_place        = apply_filters( 'widget_title', $instance['text_pass_place'] );
    ...
    <div class="wpclient_login_block <?php echo $classes ?>">
            ...
            <div id="login">
                <form method="post" name="wpclient_login_form" id="wpclient_login_form" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>">
                    ...
                    <ul style="list-style: none;">
                        ...
                        <li>
                            <input type="text" name="wpclient_login" id="wpclient_login" placeholder="<?php echo $text_login_place ?>" value="<?php echo ( isset( $_POST['wpclient_login'] ) ) ? esc_attr( $_POST['wpclient_login'] ) : '' ?>" />
                        </li>
                        <li>
                            <label for="wpclient_pass"><?php echo $text_pass ?></label>
                        </li>
                        <li>
                            <input type="password" name="wpclient_pass" id="wpclient_pass" placeholder="<?php echo $text_pass_place ?>" value="" autocomplete="off" />
                        </li>
                        ...
                        <?php
                        $clients_staff_settings = WPC()->get_settings( 'clients_staff' );
                        if( isset( $clients_staff_settings['lost_password'] ) && 'yes' == $clients_staff_settings['lost_password'] && $enable_forgot ) { ?>
                            <li id="nav">
                            <?php $login_url = explode( '?', WPC()->get_login_url() );
                                $login_url = $login_url[0]; ?>
                                <a href="<?php echo $login_url ?>?wpc_action=lostpassword" title="Password Lost and Found"><?php echo $text_forgot ?></a>
                            </li>
                        <?php } ?>
                    </ul>
                </form>
        ...
        <?php } else {
            $link = WPC()->get_logout_url();

            if ( isset( $logout_redirect ) && '' != $logout_redirect )
                $link = add_query_arg( array( 'redirect_to' => $logout_redirect ), $link ); ?>

            <p><span><?php echo do_shortcode( $text_welcome ); ?></span></p>
            <p><a href="<?php echo $link ?>" ><?php echo $text_logout ?></a></p>
        <?php } ?>
    ...
    function update( $new_instance, $old_instance ) {
        ...
        $instance['text_login_place']       = strip_tags( $new_instance['text_login_place'] );
        $instance['text_pass']              = strip_tags( $new_instance['text_pass'] );
        $instance['text_pass_place']        = strip_tags( $new_instance['text_pass_place'] );
        ...
        $instance['enable_forgot']          = ( isset( $new_instance['enable_forgot'] ) )
                ? strip_tags( $new_instance['enable_forgot'] ) : 0;
        $instance['disable_hub_redirect']   = ( isset( $new_instance['enable_forgot'] ) ) 
                ? strip_tags( $new_instance['disable_hub_redirect'] ) : 0;
        return $instance;
    }
    ...
    function form( $instance ) {
        ...
        $text_login_place = '';
        if ( isset( $instance['text_login_place'] ) )
            $text_login_place = esc_attr( $instance['text_login_place'] );
        ...
        $text_pass_place = '';
        if ( isset( $instance['text_pass_place'] ) )
            $text_pass_place = esc_attr( $instance['text_pass_place'] );
        ...
        ?>
            ...
            <p>
                <label for="<?php echo $this->get_field_name( 'text_login_place' ); ?>"><?php _e( 'Login Placeholder Text:', WPC_CLIENT_TEXT_DOMAIN ) ?></label>
                <input class="widefat" id="<?php echo $this->get_field_id( 'text_login_place' ); ?>" name="<?php echo $this->get_field_name( 'text_login_place' ); ?>" type="text" value="<?php echo $text_login_place; ?>" />
            </p>
            <p>
            ...
            <p>
                <label for="<?php echo $this->get_field_name( 'text_pass_place' ); ?>"><?php _e( 'Password Placeholder Text:', WPC_CLIENT_TEXT_DOMAIN ) ?></label>
                <input class="widefat" id="<?php echo $this->get_field_id( 'text_pass_place' ); ?>" name="<?php echo $this->get_field_name( 'text_pass_place' ); ?>" type="text" value="<?php echo $text_pass_place; ?>" />
            </p>
            <p>
...
add_action( 'widgets_init', function() { return register_widget( "wpc_client_widget" ); } );