...
/*
   Class: ServiceMasterMikadoFramework
...
class ServiceMasterMikadoFramework {
	...
	private function __construct() {
		$this->mkdOptions   = ServiceMasterMikadoOptions::get_instance();
		$this->mkdMetaBoxes = ServiceMasterMikadoMetaBoxes::get_instance();
	}
	...
	public function setSkin(ServiceMasterMikadoSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class ServiceMasterMikadoSkinManager
 *
...
/**
 * Class ServiceMasterMikadoSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class ServiceMasterMikadoSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	/**
	 * Loads template part with params. Uses servicemaster_mikado_find_template_path function which is child theme friendly
	 *
	...
	public function loadTemplatePart($template, $params = array()) {
		...
		if($template !== '') {
			include(servicemaster_mikado_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.mkdSCIcon = '<?php echo servicemaster_mikado_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
...
/*
   Class: ServiceMasterMikadoOptions
...
class ServiceMasterMikadoOptions {
	...
	public function getOptionValue($key) {
		...
		if(array_key_exists($key, $servicemaster_options)) {
			return $servicemaster_options[$key];
...
/*
   Class: ServiceMasterMikadoAdminPage
   A class that initializes Mikado Admin Page
*/

class ServiceMasterMikadoAdminPage implements iServiceMasterMikadoLayoutNode {
	...
	function __construct($slug = "", $title = "", $icon = "") {
		$this->layout  = array();
		$this->factory = new ServiceMasterMikadoFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild(iServiceMasterMikadoRender $child) {
		$child->render($this->factory);
	}
}

/*
   Class: ServiceMasterMikadoMetaBoxes
...
/*
   Class: ServiceMasterMikadoMetaBox
   A class that initializes Mikado Meta Box
*/

class ServiceMasterMikadoMetaBox implements iServiceMasterMikadoLayoutNode {
	...
	public function setTitle($label) {
		...
		return $servicemaster_Framework->getSkin()->getSkinLabel().' '.$label;
	}
}

global $servicemaster_Framework;

$servicemaster_Framework = ServiceMasterMikadoFramework::get_instance();
$mkdSkinManager                 = new ServiceMasterMikadoSkinManager();
$servicemaster_Framework->setSkin($mkdSkinManager->getSkin());
