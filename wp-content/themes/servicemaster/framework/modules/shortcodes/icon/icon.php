<?php
namespace ServiceMaster\Modules\Shortcodes\Icon;

use ServiceMaster\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package ServiceMaster\Modules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
	...
	public function vcMap() {
		vc_map(array(
			'name'                      => esc_html__('Icon', 'servicemaster'),
			'base'                      => $this->base,
			'category'                  => 'by MIKADO',
			...
			'params'                    => array_merge(
				...
				array(
					...
					array(
						'type'        => 'textfield',
						'admin_label' => true,
						'heading'     => esc_html__('Custom Size (px)', 'servicemaster'),
					...
					array(
						'type'        => 'dropdown',
						'heading'     => esc_html__('Shadow', 'servicemaster'),
					...
					array(
						...
						'description' => esc_html__('Please insert border radius(Rounded corners) in px. For example: 4 ', 'servicemaster'),
						'dependency'  => array(
							'element' => 'type',
							'value' => array('square')
						)
                    ...
                    array(
                        'type'        => 'textfield',
                        'heading'     => esc_html__('Line height (px)', 'servicemaster'),
					...
					array(
						...
						'admin_label' => true,
						'description' => esc_html__('Enter just number. Omit pixels', 'servicemaster'),
						'dependency'  => array(
							'element' => 'type',
							'value' => array('circle', 'square')
						)
					...
					array(
						'type'        => 'colorpicker',
						'heading'     => esc_html__('Hover Icon Color', 'servicemaster'),
					...
					array(
						...
						'param_name'  => 'margin',
						'admin_label' => true,
						'description' => esc_html__('Margin (top right bottom left)', 'servicemaster')
					),
					array(
						'type'        => 'dropdown',
						'heading'     => esc_html__('Icon Animation', 'servicemaster'),
					...
					array(
						'type'        => 'textfield',
						'heading'     => esc_html__('Icon Animation Delay (ms)', 'servicemaster'),
					...
					array(
						'type'        => 'checkbox',
						'heading'     => esc_html__('Use Link as Anchor', 'servicemaster'),
						'value'       => array(esc_html__('Use this icon as Anchor?', 'servicemaster') => 'yes'),
						'param_name'  => 'anchor_icon',
						'admin_label' => true,
						'description' => esc_html__('Check this box to use icon as anchor link (eg. #contact)', 'servicemaster'),
						'dependency'  => array(
							'element' => 'link',
							'not_empty' => true
						)
	...
	public function render($atts, $content = null) {
		...
		$html = servicemaster_mikado_get_shortcode_module_template_part('templates/icon', 'icon', '', $params);
	...
	private function generateIconStyles($params) {
		...
		if (($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
			...
			if (!empty($params['custom_size'])) {
				$iconStyles[] = 'font-size:' . servicemaster_mikado_filter_px($params['custom_size']) . 'px';
			}
		}
	...
	private function generateIconHolderStyles($params) {
		...
		if (isset($params['type']) && $params['type'] !== 'normal') {
			...
			if (!empty($shapeSize)) {
				$iconHolderStyles[] = 'width: ' . servicemaster_mikado_filter_px($shapeSize) . 'px';
				$iconHolderStyles[] = 'height: ' . servicemaster_mikado_filter_px($shapeSize) . 'px';
			}

            if ($params['line_height'] !== '') {
                $iconHolderStyles[] = 'line-height: ' . servicemaster_mikado_filter_px($params['line_height']) . 'px';
            } else {
                $iconHolderStyles[] = 'line-height: ' . servicemaster_mikado_filter_px($shapeSize) . 'px';
            }
			...
			if (!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
				...
				$iconHolderStyles[] = 'border-width: ' . servicemaster_mikado_filter_px($params['border_width']) . 'px';
			...
			if ($params['type'] == 'square') {
				if (isset($params['border_radius']) && $params['border_radius'] !== '') {
					$iconHolderStyles[] = 'border-radius: ' . servicemaster_mikado_filter_px($params['border_radius']) . 'px';
				}
			}
		}
